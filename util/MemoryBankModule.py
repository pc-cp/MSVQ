...
class MemoryBankModule(torch.nn.Module):
    ...
    def __init__(self, size: int = 2**16):
        ...
        self.register_buffer(
            'labels', -1 * torch.ones(self.size).long())
    ...
    def _init_memory_bank(self, dim: int):
        ...
        Args:
            dim:
                The dimension of the which are stored in the bank(feature_dim).
    ...
    def forward(
        ...
        Args:
            ...
            labels:
                The label corresponding to the current batch of images, Self-supervised situations are not used.
        ...
        if update:
            self._dequeue_and_enqueue(output, labels=labels)
